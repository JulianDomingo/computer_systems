...
* A computer spends **most of its time copying data between memory, I/O devices, and CPU registers**. Thus, the memory hierarchy is crucial to optimal performance, which **programmers can use to their advantage / exploit to optimize the performance of their C programs**.
...
* Amdahl's Law:
    * Consider an application requiring time T<sub>old</sub> to execute, requiring some part of the computer a fraction α of this time, and that we improve its performance by a factor of **k**. Thus:

T<sub>new</sub> = (1 - α)T<sub>old</sub> + \frac{(αT<sub>old</sub>)}{k}

And can compute the speedup **S** as \frac{T<sub>old</sub>}{T_new}, or:

S = \frac{1}{(1 - α) + \frac{α}{k}}

* Multiprocessor system: a system consisting of multiple processors all under the control of a single OS kernel
    * Have several CPUs (**cores**) integrated onto a single IC chip
* **Thread-level Concurrency**: 
    * **Hyperthreading**: technique allowing a single CPU to execute multiple flows of control. This enables the CPU to take better advantage of its processing resources.
        * For example, if one thread has to wait for data to load into a cache, another thread can proceed with execution
            * Intel's i7 processor can have each CPU / core executing 2 threads each.
    * 
* **Instruction-level Parallelism**:
    * Execution of multiple instructions at a time (per clock cycle)
* **Single-instruction, Multiple-data Parallelism (SIMD)**: 
    * Modern processors have special HW allowing a single instruction to cause multiple operations to be performed in parallel
    * For example, modern processors have instructions that can add 8 pairs of single precision floating point numbers in parallel 
* **Abstractions**:
    * The instruction set archiecture (ISA) is an abstraction of the actual processor's HW
        * Illusion that a machine-code program is performing one instruction at a time sequentially, but in actuality the underlying HW executes multiple instructions in parallel, but always in a consistent way with the simple, sequential model of an ISA
    * Files are an abstraction of I/O devices
    * VM is an abstraction of the entire computer
